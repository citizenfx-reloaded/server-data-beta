...
RegisterNetEvent('sessionHostResult')
    ...
    if(GetCurrentResourceName() == resourceName) then
        Citizen.CreateThread(function()
            ...
            while attempts < 3 do
                ...
                local gamesFound = NetworkGetNumberOfGames()
                ...
                if needsToHost then
                    ...
                    if sessionHostResult == 'conflict' and gamesFound > 0 then
                        ...
                        Citizen.Trace("session creation conflict\n")
                        goto endLoop
                    end

                    -- we got a green light to host; start hosting
                    if not NetworkHostGameE1(16, false, 32, false, 0, 0) then
                        Citizen.Trace("session creation failure from NetworkHostGameE1\n")
                    ...
                    if not NetworkHostGameSucceeded() then
                        Citizen.Trace("session creation failure from NetworkHostGameSucceeded\n")
                        error('failed to initialize session')
                    end
